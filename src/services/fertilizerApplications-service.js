...
const Repository = require("../database/models/fertilizerApplications");
    ...
    async Create(userInputs, id, newItem, objectId) {
        ...
        if (results.status) {
            for (const operation of results.data.operations) {
                if (operation.operationItems.dateOfApplication) {
                    const { status, msg } = await operationDate(
                        {
                            id: operation.objectId,
                            dateOfOperation: operation.operationItems.dateOfApplication,
                            seasonId: results.data.seasonId,
                        },
                        POP_RESOURCES_DATA.fertilizerApplications + ` (${operation.operationName})`
                    )
                    if (!status) {
