const Repository = require("../database/models/seedtreatmenttypes");
const seedtreatmentNamesRepository = require("../database/models/seedtreatmentnames");
...
class Service {
    ...
    async HardDelete(id) {
        ...
        if (data) {
            const seednames = await seedtreatmentNamesRepository.find({ seedTreatmentTypeId: id });
            if (seednames && seednames.length) {
                for (const seed of seednames) {
                    await seedtreatmentNamesRepository.findOneAndDelete({ id: seed.id });
                }
            } 
