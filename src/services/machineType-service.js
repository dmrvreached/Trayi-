const Repository = require("../database/models/machineType");
const machineBrandRepository = require("../database/models/machineBrand");
...
class Service {
    ...
    async HardDelete(id) {
        ...
        if (data) {
            const machinebrands = await machineBrandRepository.find({ machineTypeId: id });
            if (machinebrands && machinebrands.length) {
                for (const mcbrand of machinebrands) {
                    await machineBrandRepository.findOneAndDelete({ id: mcbrand.id });
                }
            } 
