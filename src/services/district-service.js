const Repository = require("../database/models/district");
...
class Service {
    ...
    async Create(userInputs, id) {
        const exist = await Repository.findOne({ name: userInputs.name, stateId: userInputs.stateId });
        if (exist) {
    ...
    async HardDelete(id) {
        ...
        if (data) {
            const blocks = await BlockRepository.find({ districtId: id });
            if (blocks && blocks.length) {
                for (const block of blocks) {
                    await BlockRepository.findOneAndDelete({ id: block.id });
                }
            }
            const villages = await VillageRepository.find({ districtId: id });
    ...
    async Get(query) {
        ...
        if (data) {
            const newData = data.map(i => { return { ...i._doc, districtId: i.id } })
