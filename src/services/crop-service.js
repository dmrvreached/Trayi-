const Repository = require("../database/models/crops");
const { removeDuplicateObjects } = require('../utils');

class Service {

    async Create(userInputs, id) {
        const exist = await Repository.findOne({ name: userInputs.name, cropTypeId: userInputs.cropTypeId });
        if (exist) {
    ...
    async Multi(query) {
        ...
        const data = await Repository.find({
            cropTypeId: { $in: query } // Find documents where cropTypeId is in the array query
        });
        if (data) {
