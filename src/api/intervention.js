const StateService = require("../services/intervention-service");
...
module.exports = (app) => {
  ...
  app.post("/intervention", UserAuth, async (req, res, next) => {
    try {
      const { id } = req.query;
  ...
  app.delete("/intervention", UserAuth, async (req, res, next) => {
    try {
      const { id } = req.query;
  ...
  app.get("/interventions", UserAuth, async (req, res, next) => {
    try {
  ...
  app.post("/multiInterventions", UserAuth, async (req, res, next) => {
    try {
      const data = await service.Multi(req.body.cropIds)
      return res.json(data);
    } catch (err) {
      next(err);
    }
