const StateService = require("../services/machineType-service");
...
module.exports = (app) => {
  ...
  app.post("/machineType", UserAuth, async (req, res, next) => {
    try {
      const { id } = req.query;
  ...
  app.delete("/machineType", UserAuth, async (req, res, next) => {
    try {
      const { id } = req.query;
      const data = await service.HardDelete(id);
      return res.json(data);
    } catch (err) {
      console.log(err,"==errmachinetypes");
      next(err);
    }
  });

  app.get("/machineTypes", UserAuth, async (req, res, next) => {
    try {
