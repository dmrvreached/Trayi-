const Service = require("../services/district-service");
...
module.exports = (app) => {
  ...
  app.post("/district", UserAuth, async (req, res, next) => {
    try {
      const { id } = req.query;
  ...
  app.delete("/district", UserAuth, async (req, res, next) => {
    try {
      const { id } = req.query;
  ...
  app.get("/districts", UserAuth, async (req, res, next) => {
    try {
  ...
  app.post("/multidistricts", UserAuth, async (req, res, next) => {
    try {
      const data = await service.Multi(req.body.stateIds)
      return res.json(data);
    } catch (err) {
      next(err);
    }
