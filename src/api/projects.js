...
module.exports = (app) => {
    ...
    app.delete("/projects", UserAuth, async (req, res, next) => {
    try {
        const { id } = req.query;
    ...
    app.get("/projects", UserAuth, async (req, res, next) => {
    try {
    ...
    app.get("/dcprojects", UserAuth, async (req, res, next) => {
        try {
            const data = await service.GetDcData(req.query);
            return res.json(data);
        } catch (err) {
            next(err);
        }
